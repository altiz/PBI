...
IS
BEGIN
        INSERT INTO pbi.VALUE_PLAN (MAIN_ID, CALENDAR_ID, VALUE, "YEAR", IS_CURR)
        WITH dat AS (
        ...
        SELECT ct.COB_ID,
            ...
            bc.EXPENCE_KIND,
            CASE 
                WHEN biv."YEAR" = t."YEAR" then  biv.value 
            END AS VALUE_CURR_1_EL,
            CASE 
                WHEN biv."YEAR" = t."YEAR"+1 then  biv.value 
            END AS VALUE_CURR_2_EL,
            CASE 
                WHEN biv."YEAR" = t."YEAR"+2 then  biv.value 
            END AS VALUE_CURR_3_EL,
            CASE 
                WHEN biv."YEAR" = t."YEAR"+3 then  biv.value 
        ...
        GROUP BY  COB_ID,
            ...
            M_YEAR,
            financing_source_id),
        sum_trans AS (SELECT financing_source_id, COB_ID, M_DATE, NULLIF(VALUE_CURR_1,0) AS VALUE, M_YEAR AS YEAR, 1 AS IS_CURR FROM curr_sum
        UNION ALL
        SELECT financing_source_id, COB_ID, M_DATE, NULLIF(VALUE_CURR_2,0) AS VALUE, M_YEAR+1, null FROM curr_sum
        UNION ALL
        SELECT financing_source_id, COB_ID, M_DATE, NULLIF(VALUE_CURR_3,0) AS VALUE, M_YEAR+2, null FROM curr_sum
        UNION ALL
        SELECT financing_source_id, COB_ID, M_DATE, NULLIF(VALUE_CURR_4,0) AS VALUE, M_YEAR+3, null FROM curr_sum)
        SELECT m.ID AS main_id,
            ...
            vp.IS_CURR
        FROM pbi.COB c 
        JOIN pbi.TITLE t3 ON t3.COB_ID = c.id
        JOIN pbi.v3_main m ON m.TITLE_NUMBER = t3.TITLE_NUMBER 
        JOIN pbi.MAIN_CALENDAR_LINK_ALT mcla ON mcla.MAIN_ID = m.ID 
        JOIN STROY.sum_trans vp ON vp.COB_ID = c.ID 
            and vp.M_DATE = mcla.CALENDAR_ID 
            AND vp.FINANCING_SOURCE_ID = m.FINANCING_SOURCE_ID;
    commit;
END GET_VALUE_PLAN;
